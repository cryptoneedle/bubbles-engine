...
/**
 * <p>description: 通用错误反馈处理器 </p>
 * <p/>
 * <p/>
 * 以下打钩的是有具体实现或关联的状态码
 * <p/>
 * 1XX     信息       Informational          服务器收到请求，需要客户端继续执行操作。通常是服务器内部使用的中间状态，不向用户显式
 * 2xx  √  成功       Success                请求被成功接收并处理
 * 3xx     重定向     Redirection            客户端需进一步的操作以完成请求
 * 4xx  √  客户端错误  Client Error           客户端发出包含语法错误或无法完成的请求
 * 5xx  √  服务器错误  Server Error           服务器在处理请求的过程中发生了错误
 * <p/>
 * 1XX 状态码
 * 100     Continue                         继续。客户端应继续其请求
 * 101     Switching Protocols              切换协议。服务器根据客户端的请求切换协议。只能切换到更高级的协议，例如，切换到HTTP的新版本协议
 * 102     Processing                       正在处理。正在处理请求，但未处理完成
 * 103     Checkpoint
 * <p/>
 * 2xx 状态码
 * 200  √  OK                               请求成功。
 ...
 * 203     Non Authoritative Information    非授权信息。请求成功，但返回的meta信息不在原始的服务器，而是一个副本。常用于缓存资源
 * 204  √  No Content                       无内容。服务器成功处理，但未返回内容。在未更新网页的情况下，可确保浏览器继续显示当前文档
 * 205     Reset Content                    重置内容。服务器处理成功，用户终端（例如：浏览器）应重置文档视图。可通过此返回码清除浏览器的表单域
 * 206     Partial Content                  部分内容。服务器成功处理了部分GET请求
 * 207     Multi-Status | Partial Update OK
 ...
 * 307     ode 307: Temporary Redirect      临时重定向。与302类似。使用GET请求重定向
 * 308     ode 308: Permanent Redirect
 * <p/>
 * 4xx 状态码
 * 400     Bad Request                      客户端请求的语法错误，服务器无法理解
 * 401  √  Unauthorized                     未经授权。请求要求用户的身份认证
 ...
 * 418     I'm a teapot
 * 419     Insufficient Space on Resource | Proxy Reauthentication Required
 * 420     Method Failure
 ...
 * <p/>
 * 参考{@link org.apache.hc.core5.http.HttpStatus} (主)
 * 参考{@link org.dromara.hutool.http.meta.HttpStatus} (次)
 *
 * @author CryptoNeedle
 * @date 2024-12-11
 */
public interface Feedbacks {
    ...
    /**
     * 412 客户端请求信息的先决条件错误，未经授权
     */
    PreconditionFailedFeedback PRECONDITION_FAILED = new PreconditionFailedFeedback("客户端请求信息的先决条件错误");
    ...
    NotImplementedFeedback DISCOVERED_UNRECORDED_ERROR_EXCEPTION = new NotImplementedFeedback("发现尚未记录的 Exception 错误,建议在错误体系中添加");
     ...
     * 6xx 为数据操作相关错误
     */
    CustomizeFeedback TRANSACTION_ROLLBACK = new CustomizeFeedback("数据事务处理失败，数据回滚", FeedbackConstants.CUSTOMIZED_DATA);
    CustomizeFeedback BAD_SQL_GRAMMAR = new CustomizeFeedback("低级SQL语法错误，检查SQL能否正常运行或者字段名称是否正确", FeedbackConstants.CUSTOMIZED_DATA);
    CustomizeFeedback DATA_INTEGRITY_VIOLATION = new CustomizeFeedback("该数据正在被其它数据引用，请先删除引用关系，再进行数据删除操作", FeedbackConstants.CUSTOMIZED_DATA);
    
    /**
     * 7xx 基础设施交互错误
     */
    CustomizeFeedback PIPELINE_INVALID_COMMANDS = new CustomizeFeedback("Redis管道包含一个或多个无效命令", FeedbackConstants.CUSTOMIZED_FRAMEWORK);
}